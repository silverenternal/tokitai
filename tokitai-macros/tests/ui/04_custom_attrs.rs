...
async fn main() {
    ...
    // 调用测试
    let result = processor.call_tool("process_data", &serde_json::json!({"input": "test"})).await.unwrap();
    assert_eq!(result, "Processed: test");
    
    let result = processor.call_tool("transform", &serde_json::json!({"data": [1, 2, 3]})).await.unwrap();
