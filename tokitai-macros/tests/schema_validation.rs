...
fn test_json_schema_contains_one_of() {
    ...
    assert!(enum_vals.contains(&serde_json::Value::String("admin".to_string())), "enum 应该包含 admin");
    assert!(enum_vals.contains(&serde_json::Value::String("user".to_string())), "enum 应该包含 user");
    assert!(enum_vals.contains(&serde_json::Value::String("guest".to_string())), "enum 应该包含 guest");
}
...
fn test_json_schema_contains_pattern() {
    ...
    // 验证 pattern 字段存在
    assert!(email_schema.get("pattern").is_some(), "email 字段应该包含 pattern");
    assert_eq!(email_schema["pattern"].as_str().unwrap(), "@", "pattern 应该是 @");
}
...
fn test_json_schema_contains_min_max() {
    ...
    assert!(age_schema.get("minimum").is_some(), "age 字段应该包含 minimum");
    assert!(age_schema.get("maximum").is_some(), "age 字段应该包含 maximum");
    assert_eq!(age_schema["minimum"].as_f64().unwrap(), 0.0, "minimum 应该是 0");
    assert_eq!(age_schema["maximum"].as_f64().unwrap(), 150.0, "maximum 应该是 150");
}
...
fn test_json_schema_contains_min_max_length() {
    ...
    assert!(name_schema.get("minLength").is_some(), "name 字段应该包含 minLength");
    assert!(name_schema.get("maxLength").is_some(), "name 字段应该包含 maxLength");
    assert_eq!(name_schema["minLength"].as_u64().unwrap(), 3, "minLength 应该是 3");
    assert_eq!(name_schema["maxLength"].as_u64().unwrap(), 20, "maxLength 应该是 20");
}
...
fn test_json_schema_contains_multiple_of() {
    ...
    // 验证 multipleOf 字段存在
    assert!(count_schema.get("multipleOf").is_some(), "count 字段应该包含 multipleOf");
    assert_eq!(count_schema["multipleOf"].as_f64().unwrap(), 5.0, "multipleOf 应该是 5");
}

#[test]
fn test_tool_count() {
    // 验证工具数量
    assert_eq!(ValidationTools::TOOL_DEFINITIONS.len(), 3, "应该有 3 个工具");
}
