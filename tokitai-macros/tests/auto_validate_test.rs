...
impl UserService {
    ...
    pub fn search_products(
        ...
        Ok(format!("搜索：{} ({})", keyword, _category.unwrap_or_default()))
    }
}
...
fn test_auto_validate_empty_name() {
    ...
    let result = service.call_tool("create_user", &json!({
        "name": "",
...
fn test_one_of_validation() {
    ...
    // 无效的 status
    let result = service.call_tool("process_order", &json!({
...
fn test_min_max_length_validation() {
    ...
    // keyword 长度 < 3
    let result = service.call_tool("search_products", &json!({
        "keyword": "ab",
        "category": null,
        "price_range": null
    }));
