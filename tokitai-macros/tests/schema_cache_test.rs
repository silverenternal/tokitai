...
fn test_tool_schema_for_basic_types() {
    let tool_defs = CustomTypeTools::tool_definitions();
    let location_tool = tool_defs.iter().find(|t| t.name == "process_location").unwrap();
    
    // 验证 schema 是有效的 JSON
    let schema_value: serde_json::Value = serde_json::from_str(&location_tool.input_schema).unwrap();
...
fn test_concurrent_different_tools() {
    ...
    #[tool]
    impl ToolSetA {
        pub fn method_a1(&self) -> String { "a1".to_string() }
        pub fn method_a2(&self) -> String { "a2".to_string() }
    }
    ...
    #[tool]
    impl ToolSetB {
        pub fn method_b1(&self) -> String { "b1".to_string() }
        pub fn method_b2(&self) -> String { "b2".to_string() }
    }
...
fn test_schema_is_valid_json() {
    ...
    for tool_def in tool_defs {
        ...
        assert!(result.is_ok(), "Schema 应该是有效的 JSON: {}", tool_def.input_schema);
    }
}
...
fn test_schema_required_fields() {
    ...
    let schema_value: serde_json::Value = serde_json::from_str(&create_user_tool.input_schema).unwrap();
...
fn test_empty_param_tool() {
    ...
    #[tool]
    impl EmptyTool {
        pub fn no_params(&self) -> String { "no params".to_string() }
    }
...
fn test_single_param_tool() {
    ...
    impl SingleParamTool {
        pub fn single_param(&self, value: String) -> String { value }
    }
