...
| `#[wrap]`                | Curate which methods of a client struct to expose | `new(client)` + the same artifacts as `#[tool]`              | Yes          | Same as `#[tool]`                                       | `examples/deprecated/wrap_native.rs` (placeholder)                            |
| `#[openapi]` / `#[openapi_op]` | Wrap a whole REST API from an OpenAPI 3 spec | `phf::Map<operationId, Op>` + the same per-method artifacts    | Yes          | Same as `#[tool]`; no spec parsing at runtime          | `examples/wrap_openapi.rs`                                                   |
| `#[delegate]`            | Re-expose an inner method with no body          | Forwarded body + `__TOOL_DEF_*` + `__call_*` (no dispatcher)  | Yes          | Zero (the body is a single field access)                | `examples/deprecated/delegate_method.rs` (placeholder)                        |
| `#[retry]`               | Make a flaky call self-healing                  | A `loop { … }` around the body with backoff                   | Yes          | One `Result` match + (optional) sleep per attempt       | `examples/deprecated/resilient_tool.rs` (placeholder)                        |
| `#[rate_limit]`          | Throttle an external API                        | A token-bucket pre-guard                                      | Yes          | One atomic CAS per call (lock-free)                     | Same as above                                                                |
| `#[circuit_breaker]`     | Stop hammering a known-broken dependency        | A 3-state machine, updated pre/post                           | Yes          | Three `AtomicU8/U32/U64` loads/stores per call          | Same as above                                                                |
