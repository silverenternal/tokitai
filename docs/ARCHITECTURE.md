...
│   1. 解析 impl 块和 pub 方法                                   │
│   2. 提取方法签名和文档注释                                   │
│   3. 生成 `tool_definitions()` 方法（编译期 `LazyLock` 初始化）           │
│   4. 生成 call_tool 分发器                                   │
│   5. 生成参数解析辅助函数                                    │
└─────────────────────────────────────────────────────────────┘
                              ↓
...
```rust
impl Calculator {
    // ① 工具定义（v0.4.0+ 是 `tool_definitions()` 方法，编译期 `LazyLock` 初始化；v0.5.0 沿用）
    ...
    // ③ Per-method wrapper (generated). Parses the JSON args, runs
