...
- **LLM-powered capability inference (T-036, M-0.12.0).** New `tokitai-llm infer-capabilities` subcommand reads tool definitions from `--schema` and asks the LLM to suggest `requires = [...]` capability strings (e.g. `"db:read"`, `"fs:write"`, `"net:http"`) for each tool. Outputs JSON Lines with per-tool capability suggestions suitable for piping into build scripts or manual review.

- **LLM-powered example generation (T-037, M-0.12.0).** `tokitai-llm examples` gains `--provider` and `--schema` flags. When configured, the subcommand calls the LLM to generate one realistic example call per tool and attaches the result as T-016-compatible `baked_examples` on the emitted envelopes. Falls back gracefully to the existing stub path when no provider is configured.

- **Macro-side LLM integration hooks (T-034 macro side).** `tokitai-macros` gains `src/tool/llm/mod.rs` with three integration points: (1) `emit_verify_warnings` reads a JSON report file (path via `TOKITAI_LLM_VERIFY_REPORT`) at proc-macro time and emits `cargo:warning=` lines for every finding — zero cost when the env var is unset; (2) `emit_cache_key_const` outputs a hidden `pub const __TOKITAI_LLM_CACHE_KEY: &str` (stable `DefaultHasher`-based hash of the schema, opt-in via `#[tool(llm_cache_key = true)]`); (3) `llm_aware_wrapper` wraps every generated `__call_*` body in a `tracing` span (enabled via `TOKITAI_LLM_HOOK=1`). All hooks short-circuit to zero cost on the default build path. `ToolMethodInfo` gains `#[derive(Clone)]` for test infrastructure.
